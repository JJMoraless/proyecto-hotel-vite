...
const getUsuarios = async () => {
    try {
        const response = await hotelApi.get("users");
...
document
    ...
    .addEventListener("click", async () => {
        ...
        if (doc.length < 8) {
            ...
            document.getElementById("userDocument").style.backgroundColor = 'lightpink';
            return;
        }
        if (
            doc.trim() === "" ||
            ...
            if (ficha.trim() === "") {
                document.getElementById("userFicha").style.backgroundColor = 'lightpink';
            }
            if (email.trim() === "") {
                document.getElementById("userEmail").style.backgroundColor = 'lightpink';
            }
            if (numberPhone.trim() === "") {
                document.getElementById("userPhone").style.backgroundColor = 'lightpink';
            }
            if (addres.trim() === "") {
                document.getElementById("userAddress").style.backgroundColor = 'lightpink';
            }
            if (role.trim() === "") {
                document.getElementById("userRole").style.backgroundColor = 'lightpink';
            }
            if (state.trim() === "") {
                document.getElementById("userState").style.backgroundColor = 'lightpink';
            }
        } else {
            const userData = {
                document: doc,
