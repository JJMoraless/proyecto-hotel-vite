...
function openReservationModal(reservationId) {
    const getReservationById = async (reservationId) => {
      const mensajeRegistro = document.getElementById('mensajeRegistro').style.display='none';
        try {
            ...
            if (reservation) {
                ...
                document.getElementById('hostDocument').value = reservation.host.document || '';
                document.getElementById('hostDocumentType').value = reservation.host.document_type || '';
                document.getElementById('hostName').value = reservation.host.name || '';
                document.getElementById('hostBirthdate').value = reservation.host.birthdayDate ? reservation.host.birthdayDate.substring(0, 10) : '';
                document.getElementById('hostPhone').value = reservation.host.numberPhone || '';
                document.getElementById('hostEmail').value = reservation.host.email || '';
                document.getElementById('hostAddress').value = reservation.host.addres || '';
                document.getElementById('hostCity').value = reservation.host.city || '';
                document.getElementById('hostCountry').value = reservation.host.country || '';
                document.getElementById('hostOccupation').value = reservation.host.occupation || '';
                document.getElementById('hostCompany').value = reservation.host.company||'';
                document.getElementById('numReserva').value = reservationId || '';

                document.getElementById('entryDate').value = reservation.dateEntry ? reservation.dateEntry.substring(0, 10) : '';
                document.getElementById('departureDate').value = reservation.dateOutput ? reservation.dateOutput.substring(0, 10) : '';
...
//Boton Modificar
document.getElementById('btnHabilitarEdicion').addEventListener('click',function() {
...
//boton Actualizar
document.getElementById('btnActualizarHuesped').addEventListener('click', async () => {
  ...
  try {
    const response = await hotelApi.put(`host/${hostDocument}`,updatedHostData);
...
document.getElementById('btnCheckIn').addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await hotelApi.post('registers', {userId, reservationId, travel_reason});
  
      // Verifica la respuesta y maneja cualquier resultado necesario
      if (response.status === 200) {
        console.log('Solicitud POST exitosa');
        //mensaje de post
        const mensajeRegistro = document.getElementById('mensajeRegistro').style.display='block';
