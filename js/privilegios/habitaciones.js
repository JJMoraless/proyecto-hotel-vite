...
document
  .getElementById('btnActualizarTarifas')
  .addEventListener('click', async () => {
    ...
    try {
      if (idTarifaEjecutivo && idTarifaRegular) {
        const responseEjecutivo = await hotelApi.put(
          `rates/${idTarifaEjecutivo}`,
          {
        ...
        if (
          responseEjecutivo.status === 200 &&
          responseRegular.status === 200
        ) {
